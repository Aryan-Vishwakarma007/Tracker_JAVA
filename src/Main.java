...
class Main {
    public static void main(String[] args) {
        ...
        if (command.equals("add")) {
            ...
            String statustask = args[1];
            String statusnow = args[2];
            updatestatus(statustask ,statusnow);
        }
    ...
    static void deletetask(String tobedeleted){
        ...
        try (BufferedReader br = new BufferedReader(new FileReader("tasks.json"))){
            String line;
            while ((line = br.readLine()) != null){
                if(line.startsWith(tobedeleted)){
                    continue;
                }
                else {
                    lines.add(line);
    ...
    static void updatestatus(String statustask,String statusnow){
        ...
        try(BufferedReader br = new BufferedReader(new FileReader("tasks.json"))){
            String line;
            while((line = br.readLine()) != null){
                if(line.equals(statustask)){
                    String finaltask = line+ " | " + statusnow;
                    lines.add(finaltask);
                }
                else {
                    lines.add(line);
